...
class ProductModelTemplatesController extends AdminController
{
    ...
    public function pullVariants(Request $request, $id)
    {
        ...
        $global_price = ProductGlobalPrice::where('template_id',$id)->first();

        if($template->product_meta !='' && $global_price!=''){
            
            $meta = json_decode($template->product_meta);

            if($meta->color && $meta->size){
                
                Artisan::call('product-model-template-variants-import', [
                    '--id' => $template->id
                ]);
            ...
            flash()->error(trans('messages.set_product_price_color_size'));
        }
