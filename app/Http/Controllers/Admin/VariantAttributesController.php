    ...
    protected function grid(Request $request, $model, $title)
    {
        ...
        $grid->add('id', trans('labels.actions'))->cell(function($value, $row) {
            return '
                ...
                <a class="btn btn-xs btn-warning" href="'.url('/admin/add-price/'.$value.'/add').'">
                    <i class="fa fa-plus"></i>
                    '.trans('actions.add_price').'
                </a>
            ';
        });
            ...
            foreach ($form->model->catalogAttributes() as $attr) {
                ...
                if($attr->value=='color'){
                    ...
                    foreach($form->model->catalogAttributeOptions($attr->id) as $attr_id => $options){
                        ...
                        if(in_array($attr_id,$color_arr)){
                            $colors .= '<li class=""><label class="btn btn-primary active" style="background:'.$options.'"><input type="checkbox" name="product_meta[color][]" id="item4" value="'.$attr_id.'" class="hidden color-check my-fav" autocomplete="off" checked>	<span class="glyphicon glyphicon-ok"></span></label></li>';
                            $count++;
                        }
                        else{
                            $colors .= '<li class=""><label class="btn btn-primary" style="background:'.$options.'"><input type="checkbox" name="product_meta[color][]" id="item4" value="'.$attr_id.'" class="hidden color-check" autocomplete="off"><span class="glyphicon glyphicon-ok"></span></label></li>';
                            $count++;
                        }
